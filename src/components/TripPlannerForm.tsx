      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-black">Your Budget</label>
            <select
              value={budget}
              onChange={(e) => setBudget(e.target.value)}
              className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none transition-all text-black"
            >
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-black">Preferred Mood</label>
            <select
              value={mood}
        ...
        <div className="space-y-2">
          <label className="block text-sm font-semibold text-black">Duration (Days)</label>
          <input
            type="number"
            min="1"
            max="30"
        ...
        <div className="space-y-2">
          <label className="block text-sm font-semibold text-black">Favorite Activities / Special Requests</label>
          <textarea
            rows={3}
            value={activities}
            onChange={(e) => setActivities(e.target.value)}
            placeholder="e.g., Hiking, Trying local street food, Photography, Staying in eco-resorts..."
            className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none transition-all resize-none text-black placeholder-slate-500"
          />
        </div>
        ...
        <div className="pt-4 flex items-center justify-end gap-4">
          <button
            ...
            onClick={onCancel}
            disabled={loading}
            className="px-6 py-3 text-black hover:bg-slate-50 rounded-lg font-medium transition-colors"
          >
            Cancel
          </button>
          <button
